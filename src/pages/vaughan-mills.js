import React, { Fragment } from "react"
import ProjectPageLayout from "../components/projectPageLayout"
import MobileProjectPageLayout from "../components/mobileProjectPageLayout"
import { isBrowser } from "react-device-detect"

const img1 = require(`../assets/images/web/vaughan-mills/1.png`)
const img2 = require(`../assets/images/web/vaughan-mills/2.png`)
const img3 = require(`../assets/images/web/vaughan-mills/3.png`)
const img4 = require(`../assets/images/web/vaughan-mills/4.png`)
const img5_1 = require(`../assets/images/web/vaughan-mills/5-1.png`)
const img5_2 = require(`../assets/images/web/vaughan-mills/5-2.png`)
const img6 = require(`../assets/images/web/vaughan-mills/6.png`)
const img7_1 = require(`../assets/images/web/vaughan-mills/7-1.png`)
const img7_2 = require(`../assets/images/web/vaughan-mills/7-2.png`)
const img8 = require(`../assets/images/web/vaughan-mills/8.png`)
const img9 = require(`../assets/images/web/vaughan-mills/9.png`)
const img10_1 = require(`../assets/images/web/vaughan-mills/10-1.png`)
const img10_2 = require(`../assets/images/web/vaughan-mills/10-2.png`)
const img11_1 = require(`../assets/images/web/vaughan-mills/11-1.png`)
const img11_2 = require(`../assets/images/web/vaughan-mills/11-2.png`)

const VaughanMills = () =>
  isBrowser ? (
    <ProjectPageLayout heading="07/Vaughan Mills">
      <Fragment>
        <img
          src={img1}
          alt="Vaughan Mills"
          style={{ width: "50%", minWidth: "35rem", maxWidth: "45rem" }}
        />
      </Fragment>
      <Fragment>
        <p>
          Vaughan Mills is a stylist who we have worked closely with across a
          variety of creative projects. The following series of images are from
          recent photoshoots featuring his vintage collection.
        </p>
      </Fragment>
      <Fragment>
        <img
          src={img2}
          alt="Vaughan Mills"
          style={{
            width: "30%",
            minWidth: "25rem",
            maxWidth: "45rem",
            marginLeft: "20%",
          }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img3}
          alt="Vaughan Mills"
          style={{ width: "60%", minWidth: "40rem", maxWidth: "60rem" }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img4}
          alt="Vaughan Mills"
          style={{
            width: "45%",
            minWidth: "40rem",
            maxWidth: "60rem",
            marginLeft: "30%",
          }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img5_1}
          alt="Vaughan Mills"
          style={{ width: "40%", minWidth: "25rem" }}
        />
        <img
          src={img5_2}
          alt="Vaughan Mills"
          style={{ width: "30%", minWidth: "20rem" }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img6}
          alt="Vaughan Mills"
          style={{
            width: "40%",
            minWidth: "30rem",
            maxWidth: "50rem",
            marginLeft: "30%",
            marginTop: "10%",
          }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img7_1}
          alt="Vaughan Mills"
          style={{ width: "30%", minWidth: "20rem", maxWidth: "30rem" }}
        />
        <img
          src={img7_2}
          alt="Vaughan Mills"
          style={{ width: "30%", minWidth: "20rem", maxWidth: "30rem" }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img8}
          alt="Vaughan Mills"
          style={{ width: "35%", minWidth: "30rem", maxWidth: "50rem" }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img9}
          alt="Vaughan Mills"
          style={{ width: "60%", minWidth: "40rem", maxWidth: "60rem" }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img10_1}
          alt="Vaughan Mills"
          style={{ width: "30%", minWidth: "20rem" }}
        />
        <img
          src={img10_2}
          alt="Vaughan Mills"
          style={{ width: "20%", minWidth: "13.33rem" }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img11_1}
          alt="Vaughan Mills"
          style={{ width: "30%", minWidth: "20rem", maxWidth: "30rem" }}
        />
        <img
          src={img11_2}
          alt="Vaughan Mills"
          style={{ width: "30%", minWidth: "20rem", maxWidth: "30rem" }}
        />
      </Fragment>
    </ProjectPageLayout>
  ) : (
    <MobileProjectPageLayout heading="07/Vaughan Mills">
      <Fragment>
        <img
          src={img1}
          alt="Vaughan Mills"
          style={{ width: "85%", marginRight: "auto" }}
        />
      </Fragment>
      <Fragment>
        <p>
          Vaughan Mills is a stylist who we have worked closely with across a
          variety of creative projects. The following series of images are from
          recent photoshoots featuring his vintage collection.
        </p>
      </Fragment>
      <Fragment>
        <img
          src={img2}
          alt="Vaughan Mills"
          style={{ width: "85%", marginLeft: "auto" }}
        />
      </Fragment>
      <Fragment>
        <img src={img3} alt="Vaughan Mills" style={{ width: "100%" }} />
      </Fragment>
      <Fragment>
        <img
          src={img4}
          alt="Vaughan Mills"
          style={{ width: "85%", marginRight: "auto" }}
        />
      </Fragment>
      <Fragment>
        <img src={img5_1} alt="Vaughan Mills" style={{ width: "100%" }} />
      </Fragment>
      <Fragment>
        <img src={img5_2} alt="Vaughan Mills" style={{ width: "100%" }} />
      </Fragment>
      <Fragment>
        <img
          src={img6}
          alt="Vaughan Mills"
          style={{ width: "85%", marginRight: "auto" }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img7_1}
          alt="Vaughan Mills"
          style={{ width: "85%", marginLeft: "auto" }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img7_2}
          alt="Vaughan Mills"
          style={{ width: "85%", marginRight: "auto" }}
        />
      </Fragment>
      <Fragment>
        <img
          src={img8}
          alt="Vaughan Mills"
          style={{ width: "85%", marginRight: "auto" }}
        />
      </Fragment>
      <Fragment>
        <img src={img9} alt="Vaughan Mills" style={{ width: "100%" }} />
      </Fragment>
      <Fragment>
        <img src={img10_1} alt="Vaughan Mills" style={{ width: "100%" }} />
      </Fragment>
      <Fragment>
        <img src={img10_2} alt="Vaughan Mills" style={{ width: "80%" }} />
      </Fragment>
      <Fragment>
        <img
          src={img11_1}
          alt="Vaughan Mills"
          style={{ width: "85%", marginRight: "auto" }}
        />
      </Fragment>
      <Fragment>
        <img src={img11_2} alt="Vaughan Mills" style={{ width: "80%" }} />
      </Fragment>
    </MobileProjectPageLayout>
  )

export default VaughanMills
